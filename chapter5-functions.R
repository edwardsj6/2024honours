...
generate_report <- function(files, is_diann=F, map_concs =T) {
  ...
  useful_columns_diann <- c('Run', "Genes", "Protein.Group", 
                            ...
                            'Stripped.Sequence','PG.Q.Value', 'PEP', 'RT','Precursor.Id'
  ...
  #add columns
  Report <- Report %>%
    separate(R.FileName, c('MS', 'window', 'agc', 'it', 'rep'), remove = F, sep='_') %>% 
    mutate(
      log2 = log(Report$PG.Quantity, 2), 
      MS= 'Astral',
      ID = paste0(window, '_', agc,'_', it)
    ) 
...
generate_cvs <- function(data) {
  ...
  cv_report <- cv_report %>% select(R.FileName, PG.Genes, log2, ID_gene)
  cv_report$unique_id <- paste(cv_report$R.FileName, cv_report$PG.Genes, cv_report$log2)
...
generate_summary_report <- function(data_in, filter_levels) {
  ...
  for (meth in methods) {
    ...
    data <- Report_filt[Report_filt$MS_method == meth,] 
...
plot_mz_windows <- function(mz_list) {
  ...
  # Calculate the number of windows and set the time increment
  num_windows <- nrow(mz_df)
  time_increment <- 1 / num_windows
  
  #  Add a time relative to the full cycle on the y-axis
  mz_df <- mz_df %>%
    mutate(time = 1 - seq(0, 1 - time_increment, by = time_increment),
           type = "MS2")  # Label for legend
  
  # Create an additional rectangle at the top spanning the entire m/z range
  additional_rect <- data.frame(
    mz_start = 380, 
    mz_end = 980, 
    ymin = 1, 
    ymax = 1 + time_increment,  # Slightly above the last rectangle
    type = "MS1"  # Label for legend
  )
  
  # Combine the data
  all_rects <- bind_rows(
    mz_df %>% mutate(ymin = time - time_increment, ymax = time),
    additional_rect
  )
  
  # Plotting the m/z ranges as rectangles with flipped time axis
  p <- ggplot(all_rects, aes(xmin = mz_start, xmax = mz_end, ymin = ymin, ymax = ymax, fill = type)) +
    geom_rect(alpha = 1, colour='black', size=.1) +
    scale_x_continuous(breaks = seq(380, 980, by = 100), limits = c(380, 980.7)) +
    scale_y_continuous(name = "Time Relative to Full Cycle", breaks = seq(0, 1 + time_increment, by = 0.1)) +
